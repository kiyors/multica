 ...
 */
export function LabelsPanel({ projectId }: { projectId?: string }) {
  const { t } = useT("issues");
  ...
  const saveEdit = (id: string) => {
    ...
    setEditError("");
    update.mutate(
      { id, name, color: editColor, project_id: projectId },
      {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2 max-h-[50vh] overflow-y-auto">
        {isLoading && <p className="text-body text-muted-foreground">{t(($) => $.labels_panel.loading)}</p>}
        {!isLoading && labels.length === 0 && (
          <p className="text-body text-muted-foreground">{t(($) => $.labels_panel.empty)}</p>
        )}
        {labels.map((label) => {
            ...
            >
              ...
              {isEditing && editError && (
                <p
                  ...
                  className="mt-1.5 text-micro text-destructive"
                  role="alert"
                >
                  {editError}
                </p>
...
function ColorPalette({
  ...
  return (
    <div className={compact ? "flex items-center" : "flex items-center gap-3"}>
      {!compact && <UILabel className="text-micro text-muted-foreground">{t(($) => $.labels_panel.color_label)}</UILabel>}
      <label
