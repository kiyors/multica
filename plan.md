...
- CI follow-up for [Actions run 30887560838](https://github.com/kiyors/multica/actions/runs/30887560838): separated loopback-only local self-host port publication from the portless Dokploy/Traefik stack, retained Compose-authoritative local health checks, repaired recovered review/milestone migration ordering, reconciled the historical `139_project_members` table with migration `254_project_members`, and moved Calendar/project-tab typography onto role-named tokens. Verified with fresh and upgrade-style database migrations, all seven DB-backed approval/review/bulk-assignment tests, the self-host configuration and installer suites, and all 188 Web tests.
