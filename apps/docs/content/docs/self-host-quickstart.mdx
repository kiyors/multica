...
```bash
docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.local.yml ps
```
...
`docker compose restart` only restarts existing containers and does not re-read `.env`. After changing configuration, run `docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.local.yml up -d` to re-read `.env`.
</Callout>
...
```bash
docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.local.yml logs backend \
...
# Tail backend logs
docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.local.yml logs -f backend
...
```bash
docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.local.yml exec -T postgres \
...
| Symptom | Check first |
|---|---|
| `/readyz` does not return `ok` | Run `docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.local.yml logs backend postgres`. |
