...
| `GITHUB_APP_PRIVATE_KEY` | empty | Optional. Full PEM block of the App's RSA private key, or a single-line base64 encoding of that PEM. Use base64 in Docker/Dokploy env files. Used to mint the short-lived JWT GitHub requires for App-authenticated REST calls |
