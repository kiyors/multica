    ...
    <main className="flex min-h-screen flex-col bg-background text-foreground lg:h-screen lg:flex-row">
      <section className="flex min-h-[55vh] flex-1 flex-col bg-black">
        <header className="flex h-14 items-center justify-between border-b border-white/10 px-4 text-white">
          <div className="min-w-0"><p className="truncate text-body font-medium">{review.asset.name}</p><p className="text-micro text-white/60">Guest media review</p></div>
          {isPdf && <div className="flex items-center gap-1"><button aria-label="Previous page" className="rounded p-2 hover:bg-white/10" disabled={pageIndex === 0} onClick={() => setPageIndex((value) => Math.max(0, value - 1))}><ChevronLeft className="h-4 w-4" /></button><span className="min-w-16 text-center text-micro">Page {pageIndex + 1} {pdfNumPages ? `/ ${pdfNumPages}` : ''}</span><button aria-label="Next page" className="rounded p-2 hover:bg-white/10" disabled={pdfNumPages !== null && pageIndex >= pdfNumPages - 1} onClick={() => setPageIndex((value) => pdfNumPages ? Math.min(pdfNumPages - 1, value + 1) : value + 1)}><ChevronRight className="h-4 w-4" /></button></div>}
        </header>
      ...
      <aside className="flex w-full flex-col border-l border-border bg-background lg:w-[360px]">
        <div className="flex items-center gap-2 border-b border-border p-4 font-medium"><MessageSquare className="h-4 w-4" />Feedback {isPdf && <span className="text-micro font-normal text-muted-foreground">on page {pageIndex + 1}</span>}</div>
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {comments.length === 0 ? <p className="py-8 text-center text-body text-muted-foreground">No feedback on this page yet.</p> : comments.map((comment) => <article key={comment.id} className="rounded-lg border border-border bg-card p-3"><p className="text-micro font-medium">{comment.guest_name ?? "Team member"}</p><p className="mt-1 whitespace-pre-wrap text-body text-muted-foreground">{comment.content}</p></article>)}
        </div>
        <form className="space-y-3 border-t border-border p-4" onSubmit={submitComment}>
          ...
          <input value={guestName} onChange={(event) => setGuestName(event.target.value)} maxLength={80} required placeholder="Your name" className="w-full rounded-md border border-border bg-background px-3 py-2 text-body" />
          <div className="flex gap-2">
            <button type="button" onClick={() => setSentiment(sentiment === "approved" ? "pending" : "approved")} className={`flex flex-1 items-center justify-center gap-1.5 rounded-md border px-3 py-2 text-micro font-medium transition-colors ${sentiment === "approved" ? "border-green-500 bg-green-50 text-green-700" : "border-border bg-background hover:bg-accent"}`}>
              <ThumbsUp className="h-3.5 w-3.5" /> Like it
            </button>
            <button type="button" onClick={() => setSentiment(sentiment === "changes_requested" ? "pending" : "changes_requested")} className={`flex flex-1 items-center justify-center gap-1.5 rounded-md border px-3 py-2 text-micro font-medium transition-colors ${sentiment === "changes_requested" ? "border-amber-500 bg-amber-50 text-amber-700" : "border-border bg-background hover:bg-accent"}`}>
              <AlertCircle className="h-3.5 w-3.5" /> Changes needed
            </button>
          </div>
          <textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={5000} required placeholder="Leave feedback…" className="min-h-24 w-full resize-y rounded-md border border-border bg-background px-3 py-2 text-body" />
          <button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-body font-medium text-primary-foreground disabled:opacity-50"><Send className="h-4 w-4" />{submitting ? "Sending…" : "Send feedback"}</button>
        </form>
      </aside>
    </main>
  );
