        ...
        .catch((err) => {
          setDesktopError(
            err instanceof Error
              ? err.message
              : t(($) => $.auth.desktop_handoff.prepare_failed),
          );
        });
      return;
    }
          ...
          <Card className="w-full max-w-sm">
            <CardHeader className="text-center">
              <CardTitle className="text-2xl">
                {t(($) => $.auth.desktop_handoff.failed_title)}
              </CardTitle>
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">
              {t(($) => $.auth.desktop_handoff.opening_title)}
            </CardTitle>
            <CardDescription>
              {desktopToken
                ? t(($) => $.auth.desktop_handoff.opening_description)
                : t(($) => $.auth.desktop_handoff.preparing)}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex justify-center">
            {desktopToken ? (
              ...
              >
                {t(($) => $.auth.desktop_handoff.open_button)}
              </Button>
            ) : (
