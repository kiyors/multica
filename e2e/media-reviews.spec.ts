...
test.describe("Media Reviews", () => {
  ...
  test("invalid guest review tokens fail closed", async ({ page }) => {
    await page.goto(`/guest/review/not-a-valid-review-token`);
    await expect(page.getByText("This review link is invalid or has expired.")).toBeVisible();
